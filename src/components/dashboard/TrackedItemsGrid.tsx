...
function TrackedItemSkeleton() {
  return <Card className="h-full animate-pulse bg-gradient-to-br from-card to-muted/30 relative overflow-hidden">
...
export function TrackedItemsGrid({
  ...
  if (isLoading) {
    return <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {Array.from({
        length: 6
      }).map((_, index) => <TrackedItemSkeleton key={index} />)}
      </div>;
  }
  if (items.length === 0) {
  ...
  return <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
      {items.map((item, index) => <div key={item.id} className="animate-fade-in-up" style={{
      animationDelay: `${index * 100}ms`
    }}>
          <TrackedItemCard item={item} onToggle={onToggleItem} onDelete={onDeleteItem} isToggling={toggleMutationState?.isPending && toggleMutationState?.variables?.id === item.id} isDeleting={deleteMutationState?.isPending && deleteMutationState?.variables === item.id} />
        </div>)}
    </div>;
}